...
import { LineChart as LineChartIcon, RotateCcw } from 'lucide-react';
...
const ChartSkeleton = () => (
  <div className="glass-panel rounded-xl h-[360px] animate-pulse" aria-hidden />
);
...
const MacroPage = () => {
  ...
  const { selectedId, range, seriesCache, isLoadingCatalog, isLoadingSeries, error, init, setRange } =
    useMacroStore();
  ...
  const isLoading = isLoadingCatalog || isLoadingSeries;
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="glass-panel rounded-xl p-8 text-center">
          <p className="text-cb-negative mb-4">{t.macro.error}</p>
          <button
            onClick={() => init()}
      ...
      {!error && isLoading && !data && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[0, 1, 2].map((i) => (
              <div key={i} className="glass-panel rounded-xl h-24 animate-pulse" aria-hidden />
            ))}
          </div>
          <ChartSkeleton />
        </div>
